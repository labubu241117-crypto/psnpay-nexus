...
const banners = [
  ...
  {
    id: 2,
    title: "Paket Data 6GB",
    subtitle: "30 Hari Aktif",
    price: "13.000",
    oldPrice: "15.000",
      ...
      <div className="relative">
        <div className="flex gap-3 overflow-x-auto px-4 pb-1 snap-x snap-mandatory" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none', WebkitOverflowScrolling: 'touch' }}>
          {banners.map((banner) => (
            <div
              key={banner.id}
              className="snap-start shrink-0 w-[75%] rounded-2xl p-4 flex items-center gap-3 cursor-pointer transition-transform hover:scale-[1.02] active:scale-[0.98]"
              style={{
                background: `linear-gradient(135deg, hsl(${banner.accent} / 0.2) 0%, hsl(${banner.accent} / 0.05) 100%)`,
                border: `1px solid hsl(${banner.accent} / 0.3)`,
              }}
            >
              <div
                className="w-14 h-14 rounded-2xl flex items-center justify-center shrink-0"
                style={{ backgroundColor: `hsl(${banner.accent} / 0.15)` }}
              >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5 mb-0.5">
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: `hsl(${banner.accent})` }}>Lebih Murah</span>
                </div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  {banner.oldPrice && (
                    <span className="text-[11px] text-muted-foreground line-through">Rp {banner.oldPrice}</span>
                  )}
